...
import numpy as np
from KQNodes.KQNodes.middlewares.slogger import SafeLogger
from KQNodes.KQNodes.funcs.iit import emd_efecto, ABECEDARY
from KQNodes.KQNodes.middlewares.profile import gestor_perfilado, profile
from KQNodes.KQNodes.funcs.format import fmt_biparticion_q
from KQNodes.KQNodes.models.base.sia import SIA
...
from KQNodes.KQNodes.constants.base import (
    COLS_IDX,
    INT_ZERO,
    TYPE_TAG,
    NET_LABEL,
...
from KQNodes.KQNodes.models.base.application import aplicacion
...
class KQNodes(QNodes):
    """
    Extensión heurística basada en k−1 biparticiones sucesivas.
    Compatible con la observación teórica del documento de que una k-partición 
    puede verse como k−1 biparticiones sucesivas.
    """
    def aplicar_estrategia_k(
        ...
        futuro = tuple((EFFECT, idx_efecto) for idx_efecto in self.sia_subsistema.indices_ncubos)
        ...
        grupos_actuales = [vertices]
        
        def group_to_side(grupo):
            alc = [idx for t, idx in grupo if t == EFFECT]
            mec = [idx for t, idx in grupo if t == ACTUAL]
            return (np.array(sorted(alc), dtype=np.int8), np.array(sorted(mec), dtype=np.int8))

        def fmt_grupo(g):
            alc = sorted([idx for t, idx in g if t == EFFECT])
            mec = sorted([idx for t, idx in g if t == ACTUAL])
        ...
        while len(grupos_actuales) < k:
            ...
            for idx_g, grupo in enumerate(grupos_actuales):
                ...
                # Ejecutar nuevamente QNodes sobre dicho grupo
                self.memoria_delta.clear()
                self.memoria_grupo_candidato.clear()
                self.clave_submodular = [], []
                
                # Convert the tuple key to list if it returns a tuple directly from memory
                mip_sub_key = self.algorithm(grupo)
                if not mip_sub_key:
                    continue
                    
                # mip_sub_key could be a tuple or single element
                if isinstance(mip_sub_key, tuple) and len(mip_sub_key) > 0 and isinstance(mip_sub_key[0], tuple):
                    mip_sub = list(mip_sub_key)
                elif isinstance(mip_sub_key, tuple):
                    mip_sub = [mip_sub_key]
                else:
                    mip_sub = list(mip_sub_key)
                
                g1 = list(mip_sub)
                g2 = list(set(grupo) - set(mip_sub))
                ...
                # Medir la mejora del EMD (minimizar la pérdida nueva es equivalente)
