...
from KQNodes.BruteForce.constants.base import BASE_TWO, COLS_IDX, INT_ZERO
from KQNodes.BruteForce.constants.error import ERROR_ESPACIOS_INCOMPATIBLES
from KQNodes.BruteForce.funcs.iit import reindexar, seleccionar_estado
from KQNodes.BruteForce.models.base.application import aplicacion
from KQNodes.BruteForce.models.core.ncube import NCube
from KQNodes.BruteForce.models.enums.notation import Notation
...
class System:
    ...
    def k_partir(
        self,
        sides: list[tuple[NDArray[np.int8], NDArray[np.int8]]],
    ) -> "System":
        """
        Generaliza la bipartición a k grupos (lados).
        Cada lado es una tupla (alcance, mecanismo).
        """
        nuevo_sistema = System.__new__(System)
        nuevo_sistema.estado_inicial = self.estado_inicial
        nuevo_sistema.memo = self.memo

        # Create a cache key using all sides
        # Using a tuple of tuples to make it hashable
        clave = tuple(
            (tuple(alcance), tuple(mecanismo)) 
            for alcance, mecanismo in sides
        )
        ...
        if clave not in self.memo:
            ...
            for cubo in self.ncubos:
                for alcance, mecanismo in sides:
                    if cubo.indice in alcance:
                        cubos_list.append(cubo.marginalizar(np.setdiff1d(cubo.dims, mecanismo)))
                        break
            self.memo[clave] = tuple(cubos_list)
